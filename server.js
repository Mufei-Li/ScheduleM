...
const DEFAULT_MODEL = String(env('LLM_MODEL', 'qwen3.7-flash'));
...
async function forwardToLLM(body) {
  ...
  // 透传思考模式开关（默认关闭以降低延迟与费用，客户端可显式覆盖）
  const enableThinking = body?.enable_thinking === undefined ? false : Boolean(body.enable_thinking);
  ...
  const resp = await fetch(upstream, {
    ...
    body: JSON.stringify({ model, messages, temperature, enable_thinking: enableThinking })
  });
...
const server = http.createServer(async (req, res) => {
  ...
  let llmModel; // /api/llm 实际调用的模型名，用于访问日志
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/llm') {
      ...
      llmModel = String((json && json.model) || DEFAULT_MODEL);
    ...
    const line = JSON.stringify({
      ...
      model: (typeof llmModel !== 'undefined' ? llmModel : undefined), // 防御式：声明被覆盖时不抛错，仅省略字段
      ms
    });
