...
- Node 代理：[server.js](server.js) 提供认证、限流、LLM 转发与 PDF 解析入口。
- Python：[main.py](pdfplumber-fastapi-service/main.py) 仅负责 pdfplumber 表格抽取（已移除 PaddleOCR）。

## 🧩 模块说明
- 前端入口：index.html、script.js、llm_parser.js、time_utils.js、style.css。
- Node 服务：server.js，提供 /api/llm、/api/parse-pdf、/api/auth/login/logout、/healthz。
