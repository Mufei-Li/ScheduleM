...
export const isValidTime = (t: any): boolean => {
...
export const formatMinutes = (m: number): string | null => {
...
export const addMinutesToTime = (t: string, delta: number): string | null => {
...
export const diffTimeMinutes = (from: string, to: string): number | null => {
...
export const shiftSlots = (slots: TimeSlot[], indices: number[], delta: number): { ok: boolean; message?: string; slots?: TimeSlot[] } => {
...
export const validateSlots = (slots: TimeSlot[]): { ok: boolean; message?: string } => {
...
export const computeShiftedSlots = (baseSlots: TimeSlot[], idx: number, type: 'start' | 'end', newValue: string, opts?: ShiftOpts): { ok: boolean; message?: string; slots?: TimeSlot[] } => {
    ...
    const nextSlots: TimeSlot[] = nextMins.map(s => {
...
export const sanitizePeriodRange = (periodRange: any): string => {
...
const normalizeOCRText = (str: any): string => {
    if (!str) return '';
    return String(str)
...
export const normalizeWeekParityToken = (raw: any): string | null => {
...
export const parseWeekString = (str: any, opts?: { maxWeek?: number }): number[] => {
    if (!str) return [];
...
export const formatWeekRanges = (weeks: number[]): string => {
...
export const formatClassAndWeeksLines = (classNames: any, weeks: number[]) => {
...
export const icsEscapeText = (val: any): string => {
    if (val == null) return '';
    return String(val)
        .replace(/\\/g, '\\\\')
        .replace(/\r\n|\r|\n/g, '\\n')
...
export const icsFoldLine = (line: any, limitBytes?: number): string => {
    const limit = Number.isFinite(limitBytes) ? Number(limitBytes) : 75;
    const s = String(line == null ? '' : line);
    if (!s) return '';

    const enc = (typeof TextEncoder !== 'undefined') ? new TextEncoder() : null;
    const byteLen = (str: string) => {
...
export const getPeriodBounds = (periodRange: any, fallbackPeriod: any, opts?: { maxPeriod?: number }): { start: number; end: number } | null => {
...
export const getTimeRangeForPeriod = (slots: TimeSlot[], periodRange: any, fallbackPeriod: any): { startPeriod: number; endPeriod: number; startTime: string; endTime: string } | null => {
