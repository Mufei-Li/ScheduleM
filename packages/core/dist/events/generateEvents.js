...
const normalizeRule_1 = require("../rules/normalizeRule");
const generateEvents = (rules, semesterStart, currentSlots) => {
    ...
    for (const r0 of arr) {
        const norm = (0, normalizeRule_1.normalizeCourseRuleInput)(r0, currentSlots);
        if (!norm.ok || !norm.rule)
            continue;
        const r = norm.rule;
        const tr = (0, timeUtils_1.getTimeRangeForPeriod)(currentSlots, r.periodRange, null);
        if (!tr)
            continue;
        const periodNum = tr.startPeriod;
        let timeOfDay = 'morning';
        if (periodNum >= 5 && periodNum <= 8)
            timeOfDay = 'afternoon';
        if (periodNum >= 9)
            timeOfDay = 'evening';
