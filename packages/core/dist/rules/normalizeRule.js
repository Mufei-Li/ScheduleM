...
exports.normalizeCourseRuleInput = exports.parseWeeksForEditor = exports.generateId = void 0;
    ...
    s.split(/[,，]/).forEach(part => {
        ...
        if (mRange) {
            ...
            if (!Number.isFinite(a) || !Number.isFinite(b))
                return;
            if (a <= 0 || a > maxWeek || b <= 0 || b > maxWeek)
                return;
            if (b < a)
                b = a;
            for (let i = a; i <= b; i++)
        ...
        if (/^\d+$/.test(p)) {
            ...
            if (Number.isFinite(n) && n > 0 && n <= maxWeek)
                set.add(n);
        }
    });
    return Array.from(set).sort((a, b) => a - b);
};
const parseWeeksForEditor = (weeksRaw) => {
    const raw = String(weeksRaw || '').trim();
    if (!raw)
        return [];
    const maxWeek = 50;
    const parsed = (0, timeUtils_1.parseWeekString)(raw, { maxWeek });
...
exports.parseWeeksForEditor = parseWeeksForEditor;
const normalizeCourseRuleInput = (input, currentSlots) => {
    const raw = input && typeof input === 'object' ? input : {};
    const name = raw.name ? String(raw.name).trim() : '';
    if (!name)
        return { ok: false, message: '课程名称不能为空' };
    const dayOfWeek = parseInt(String(raw.dayOfWeek), 10);
    if (!Number.isFinite(dayOfWeek) || dayOfWeek < 1 || dayOfWeek > 7)
        return { ok: false, message: '星期信息无效' };
    ...
    const periodRange = (0, timeUtils_1.sanitizePeriodRange)(periodSeed);
    if (!periodRange)
        return { ok: false, message: '节次不能为空' };
    const weeksRawInput = raw.weeksRaw ? String(raw.weeksRaw).trim() : '';
    const weeksFromRaw = weeksRawInput ? (0, exports.parseWeeksForEditor)(weeksRawInput) : [];
    ...
    if (!weeks || weeks.length === 0)
        return { ok: false, message: '周次不能为空或格式不正确' };
    const weeksRaw = weeksRawInput || (0, timeUtils_1.formatWeekRanges)(weeks);
    const timeRange = (0, timeUtils_1.getTimeRangeForPeriod)(currentSlots, periodRange, null);
    if (!timeRange)
        return { ok: false, message: '节次时间无效，请检查节次时间设置' };
    return {
        ok: true,
        rule: {
            id: raw.id ? String(raw.id) : (0, exports.generateId)(),
            name,
...
exports.normalizeCourseRuleInput = normalizeCourseRuleInput;
