...
function standardizeLocation(loc) {
    // Redirect to Core if available
    if (typeof window !== 'undefined' && window.ScheduleLLMCore && window.ScheduleLLMCore.StandardizeLocation) {
        return window.ScheduleLLMCore.StandardizeLocation.standardizeLocation(loc);
    }
    
    // Fallback if Core is not loaded
    if (!loc) return { location: "待通知", building: "", room: "" };
    return { location: loc, building: "", room: "" };
}
